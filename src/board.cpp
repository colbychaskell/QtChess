...
std::vector<Position> Board::getPieceMoves(const Position& piece) const {
  ...
  for(size_t row = 0; row < 8; row++) {
    for(size_t col = 0; col < 8; col++) {
      try {
        if(this->at(piece)->isValidMove(piece, Position(row,col))) { //no point in checking further if impossible move
          this->testMove(this->at(piece)->getSide(), piece, Position(row, col)); //testMove will further validate
          tempMovesList.push_back(Position(row,col)); //add to moveslist if no error
        }
      }
      catch (const std::runtime_error& e){} //ignore move if error is thrown
    }
  }
...
size_t Board::testMove(const Piece::Side& playerSide, const Position& piece, const Position& dest) const{
      ...
      if(this->at(piece)->isValidMove(piece, dest)) {
        ...
        if(this->at(piece)->getName().find('P') != std::string::npos) {
            if(this->hasPiece(dest)) {
                ...
                if(dest.column != piece.column) { //only possible with diagonal move
                  throw std::runtime_error("No piece to attack");
                }
            }
          }

        if(this->isSide(dest, this->at(piece)->getSide())) { //Invalid move if dest piece is on same side as moving piece
          throw std::runtime_error("You can't capture your own piece");
        }
        //Begin checking for pieces in the way
        else if(this->at(piece)->getName().find('N') != std::string::npos) { //Skip if piece is a knight
          return this->at(dest)->getValue();
        }
        else if(piece.row == dest.row) { //Horizontal Move
          for(size_t col = std::max(piece.column, dest.column)-1; col != std::min(piece.column, dest.column); col--) {
          ...
          for(size_t row = std::max(piece.row, dest.row)-1; row != std::min(piece.row, dest.row); row--) {
...
size_t Board::makeMove(const Piece::Side& playerSide, const Position& piece, const Position& dest) {
...
size_t Board::movePiece(const Position& piece, const Position& dest) {
    size_t newPoints = this->at(dest)->getValue(); //get value of captured piece (if exists)
    std::unique_ptr<Piece> old = std::make_unique<EmptyPiece>(); //store in case of undo
    this->pieces[piece.row][piece.column].swap(this->pieces[dest.row][dest.column]); //swap pieces
    this->pieces[piece.row][piece.column].swap(old); //swap old piece to old pointer (Empty Piece)

    //Make sure side is not in check after move
    if(isInCheck(this->at(dest)->getSide())) {
      //UNDO MOVE
      this->pieces[piece.row][piece.column].swap(old);
      this->pieces[piece.row][piece.column].swap(this->pieces[dest.row][dest.column]);
