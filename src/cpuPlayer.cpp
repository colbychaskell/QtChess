...
void CPUPlayer::makeRandomMove(Board& board) {
    ...
    for(auto piece : pieces) { //iterate through every owned piece
      ...
      if(tempMovesList.size() != 0) {
        moveMap.insert(std::pair<Position, std::vector<Position>>(Position(piece.row, piece.column), tempMovesList));
      }
    }

    //iterate and randomly select a move from the map
    size_t randPiece = size_t(rand()) % moveMap.size();
    std::map<Position, std::vector<Position>>::iterator mapRandPiece = moveMap.begin();
    ...
    try { //throws error if in check
      ...
      board.makeMove(this->side, mapRandPiece->first, mapRandPiece->second.at(randMove));
    }
    catch(const std::runtime_error& e) {
        bool moved = false;
        for (auto piece = moveMap.begin(); piece != moveMap.end(); piece++) {
            for(auto move = piece->second.begin(); move != piece->second.end(); move++) {
                if(!moved) {
                    try {
...
bool CPUPlayer::inCheckmate() const {
    return checkmate;
}

bool CPUPlayer::inStalemate() const {
    return stalemate;
}

Piece::Side CPUPlayer::getSide() const {
    return this->side;
}
