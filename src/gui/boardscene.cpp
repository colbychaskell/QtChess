...
BoardScene::BoardScene(QObject *parent, const Piece::Side &humanSide)
    : QGraphicsScene(parent),
      cpuPlayer(CPUPlayer(humanSide == Piece::Side::W ? Piece::Side::B
                                                      : Piece::Side::W)),
      gameBoard(Board()) {
  ...
  for (size_t x = 0; x < 8; x++) {
    for (size_t y = 0; y < 8; y++) {
      ...
      if (x % 2 != 0) {
        if (y % 2 == 0) {
          squares->push_back(
              new BoardSquare(nullptr, Position(y, x), QColor(247, 229, 165)));
        } else {
          squares->push_back(
              new BoardSquare(nullptr, Position(y, x), QColor(20, 95, 75)));
        }
      } else {
        if (y % 2 == 0) {
          squares->push_back(
...
Board &BoardScene::getBoard() { return this->gameBoard; }
  ...
  for (size_t x = 0; x < 8; x++) {
    for (size_t y = 0; y < 8; y++) {
      ...
      if (gameBoard.at(square)->getSide() != Piece::Side::E) {
        tempPieces.push_back(
            new PieceQt(gameBoard.at(square)->getName(), square));
...
void BoardScene::mousePressEvent(QGraphicsSceneMouseEvent *event) {
  ...
  if (event->button() == Qt::LeftButton) {
    ...
    for (auto square : *squares) {
      if (square->sceneBoundingRect().contains(
...
void BoardScene::mouseMoveEvent(QGraphicsSceneMouseEvent *event) {
  if (selectedPiece != nullptr) {
    selectedPiece->setPos(event->scenePos().x() - 25,
                          event->scenePos().y() - 25);
  }
}
...
void BoardScene::mouseReleaseEvent(QGraphicsSceneMouseEvent *event) {
  if (selectedPiece != nullptr) {
    try {
      if (event->button() == Qt::LeftButton) {
        ...
        if (mousePosition != Position(9, 9)) {
          if (cpuPlayer.getSide() == Piece::Side::W) {
            gameBoard.makeMove(
                Piece::Side::B, selectedPiece->getPosition(),
                mousePosition);  // Try to move on underlying board
            selectedPiece->movePiece(
                mousePosition);  // if movable, move on the graphics scene
          } else {
            gameBoard.makeMove(
                Piece::Side::W, selectedPiece->getPosition(),
                mousePosition);  // Try to move on underlying board
            selectedPiece->movePiece(
                mousePosition);  // if movable, move on the graphics scene
          }
      ...
      selectedPiece->movePiece(
          selectedPiece->getPosition());  // undo any movement
      selectedPiece = nullptr;
    }
  }
}
